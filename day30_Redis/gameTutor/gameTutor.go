...
func Bet(c *gin.Context) {
	...
	amount, _ := strconv.Atoi(amountStr) // 字串轉成數字

	// 在下注之前，先查詢玩家餘額，判斷用戶餘額足不足夠。
	balance, err := RC.ZScore(UserMember, user.Id).Result() // => ZSCORE `Table` UserID

	if err == redis.Nil {
		c.String(http.StatusOK, "查無此用戶，請先註冊")
		return
	}
	user.Balance = int(balance)
	if amount <= 0 {
		c.String(http.StatusOK, "下注金額需為正整數")
		return
	}
	if amount > user.Balance {
		c.String(http.StatusOK, "餘額不足")
		return
	}
...
func GetCurrentPrize(c *gin.Context) {
	c.JSON(http.StatusOK, getCurrentPrize())
}
...
func GetUserBets(c *gin.Context) {
	...
	if len(UserBets) == 0 {
		c.String(http.StatusOK, "目前沒有任何紀錄")
		return
	}
	c.JSON(http.StatusOK, UserBets)
}
